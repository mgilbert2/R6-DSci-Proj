...
<!-- DH: Nice job including this introduction to the project -->
...
<!-- DH: Two things about "simple" scraping: 1. The script will fail (including knitting) if you try to run it on a machine that's not online.  2. You have to hit the scraped server every time the script is run.  This isn't a big deal in this particular case, but might be if you're scraping many pages or calling an API.  

To avoid these problems, download the raw data (HTML file) locally, and actually scrape only if the raw data don't exist. -->

```{r}
url <- "https://www.ign.com/wikis/rainbow-six-siege/Weapons_and_Equipment"
page <- read_html(url)
table_data <- page %>%
  ## DH: Unless old code like this is genuinely valuable, clean it up
#   html_nodes("section.jsx-2191675443.jsx-380427680.jsx-28683165.wiki-section.wiki-html table") %>%
  html_table()
## DH: use a more descriptive name
...
```{r}
## DH: violates immutability; unnecessary pipe
my_table<- my_table %>%
  rename(Weapon = Name)
...
```{r}
## DH: This leftover(?) code is confusing: Why is it here, but unused? 
...
## DH: violates immutability; unnecessary pipe; spacing
my_table <- my_table %>%
...
<!-- DH: Make this a Markdown list -->
• Copper V, IV, III, II, I • Bronze V, IV, III, II, I • Silver V, IV,
III, II, I • Gold V, IV, III, II, I • Platinum V, IV, III, II, I •
Emerald V, IV, III, II, I • Diamond V, IV, III, II, I • Champions

Data comes from the upper echelon of skilled players. It is the top half
of all ranked players that provide this data. It is a set of extremely
...
## DH: unnecessary pipe; pipe should be at end of line
...
<!-- DH: I'm a little uncertain what this plot is showing us? --> 

```{r}
# Filter data for 'High Pick Rate'
## DH: Keep your idiom (base vs. tidyverse) consistent
## DH: Why do we have both `high_pick_rate_weapons` and `high_pick_data`? 
high_pick_data <- my_table_long[my_table_long$`PickRate` == "High Pick Rate", ]

## DH: What are these plots, why aren't we looking at them, and why did we construct `high_pick_data`? 
# # Plot
...
This source provides a score for each weapons recoil statistic. The purpose of this data was to provide insights over all variables effecting what makes an operator strong or weak in game play. 
They found this score by focusing on automatic-firing weapons, and looking at the 4th successive
bullets distance from the center at start of firing, they measured how
high (y) and how far to the side (x) it reached, in pixels.
...
```{r results='hide'}
## DH: naming convention for vars is lowercase
## DH: meaningful variable name: intentional missspelling of "score" or ???
SCOAR<- "1BxIHO_VHKXuNYypY6NRihik98HJN1b9d0_nZCE_l-ts"

## DH: same issues as scraping web pages above
...
```{r results='hide'}
## DH: violates immutability/confusing because this var already exists and has this value
SCOAR<- "1BxIHO_VHKXuNYypY6NRihik98HJN1b9d0_nZCE_l-ts"
...
## DH: after running through `unique()` I only have 51 rows? 
## interesting 71 obs... still some missing, 
```
After aquiring these scores,
I need to assign recoil statistics to each of the weapons in my dataset. But upon inspection there
are the missing guns besides the handguns? <!-- DH: Previous sentence garbled somehow? --> Is that fit for my research
still?
...
## DH: Consider using `n_distinct()` and just printing it out without storing in a variable
...
# Results
I want a clar visual that conveys how many weapons are associated with high pick rate operators as compared to the cooreasponding recoil score of the weapon.
I started by trying to look at frequency:
...
The temperature scale on the right as we will call it, is slightly confusing. "High recoil" interpreted in the data is actually optimal, but the phrase alone suggests "more" or "higher" recoil. So this is not the best representation of results
...
```
<!-- DH: This is a little better, but still hard to parse. I think the following is better still. --> 

```{r}
fulldf |> 
  count(PickRate, Recoil) |> 
  mutate(Recoil = as.factor(Recoil)) |> 
  complete(PickRate, Recoil, fill = list(n = 0)) |> 
  ggplot(aes(x = PickRate, y = Recoil, fill = n, label = n)) +
  geom_raster() +
...
<!-- DH: Just realized PickRate is out of order in the plots.  Need to coerce it to a factor and set the correct order manually. -->
...
```
```{r}
## DH: Violations of immutability and long lines in this chunk
...
Since my p-value is much greater than 0.05, I do not have evidence to
reject the null hypothesis. In other words, there is no statistically
significant correlation between Recoil and PickRate.

This means changes in recoil scores do not consistently relate to
changes in the ordinal pick rate categories in a way that increases or
decreases together. <!-- DH: Remember that you can't accept the null in a Fisherian hypothesis test, and need a power analysis to do a Neyman-Pearson test. --> 

However; Kendall's Tau requires that both variables be ordinal or
continuous. In my dataframe, Recoil is continuous, and PickRateNumeric
isordinal encoding of PickRate, with "Low Pick Rate" as 1, "Average Pick
Rate" as 2, and "High Pick Rate" as 3. <!-- DH: "However" indicates that you're going to argue Kendall's tau isn't actually appropriate in this case, or something like that.  What's that conclusion here? -->


# Limitations

If I had quantitative statistics for the Pick Rate column that may be
helpful to see a significance. The Pick Rate column is just 3 levels of
category which is not descriptive enough in this case to really pull any
trend out. If I had the "raw data" that Ubisoft does not formally
release then I could see exact percentages for pick ratios of each
operator per x amount of rounds in x amount of matches.

Similarly, the scores for recoil are not nearly as comprehensive as they
are on Ubisoft's end. additionally, there are many additional
attachments that aid in dampening recoil, these statistics are not
accounted for in my data.

Additionally for a next direction I should explore "Ban" Rates instead
of pick rates to see any correlation between highly-banned operators and
their weapon's recoil.

This kind of exploration would probably benefit from factor analysis as
well as mixed effects models, however my data is unfit for these
purposes. Specifically, my features are not continuous and also not
numeric. Multicollinearity: High correlations among some of these
variables are likely, given that weapon attributes are often designed to
balance each other (e.g., high damage might be balanced by high recoil).
This could make it difficult to separate out distinct factors.
